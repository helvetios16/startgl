...
std::vector<SearchFrame> AStarPathfinder::solveSteps(const GraphData &source) {
  ...
  start.g = 0;
  start.h = getDistance(graph.nodes[graph.startId], graph.nodes[graph.endId]);
  openSet.push(&start);
  ...
  while (!openSet.empty()) {
    ...
    for (const auto &edge : graph.adj[u]) {
      ...
      if (tentativeG < info[v].g) {
        ...
        info[v].g = tentativeG;
        info[v].h = getDistance(graph.nodes[v], graph.nodes[graph.endId]);
        openSet.push(&info[v]);
